...
- **`tags` اسلاگ انگلیسی lowercase بدون `#`.** مجموعهٔ مجاز این موضوع: `structure` `dependency` `boundary` `principle` `style` `domain` `communication` `async` `failure` `consistency` `scaling` `decision` `antipattern` `testing`. هر مدخل دو تا چهار برچسب.
...
**`serverless`** — سرورلس / Serverless · tags: `style` `scaling` · related: `stateless`، `event-driven-architecture`، `microservices`، `component`
...
**نکته:** `service-mesh` به `observability` ارجاع `related` می‌دهد ولی آن مدخل در مرحلهٔ ۱۵ است. `related` مشکلی ندارد چون در زمان نوشتن وجود ندارد — **پس `observability` را از `related` این مدخل بردار و در تسک ۱۶ اضافه‌اش کن.** در متن هم عبارت «مدخل رصدپذیری» را ننویس، وگرنه گارد ترتیب قرمز می‌شود.
...
- [ ] **Step 4: `observability` را به `related` مدخل `service-mesh` اضافه کن، و `related` مدخل `scalability` کریپتو را به‌روز کن؛ `node --test` → همچنان سبز**
- [ ] **Step 5: بررسی نهایی**
