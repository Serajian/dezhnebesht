...
**`swarm-cli`** — خط فرمان و API / CLI and API · tags: `operations` · related: `service`، `swarm`، `task-state`، `stack`
...
**`service-logs`** — لاگ سرویس / Service Logs · tags: `operations` · related: `swarm-cli`، `task-state`، `observability`
...
**`swarm-monitoring`** — پایش سوارم / Monitoring a Swarm · tags: `operations` · related: `observability`، `service-logs`، `healthcheck`، `global-service`
...
**`dockerfile`** — داکرفایل / Dockerfile · tags: `delivery` · related: `registry`، `image-tag`، `service`
