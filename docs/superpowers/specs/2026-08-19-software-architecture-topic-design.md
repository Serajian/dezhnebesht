...
data/
  ...
  architecture/                    ← پوشهٔ جدید
    categories.json
    roadmap.json                   ← هر ۱۵ مرحله از فاز ۱
    entries/
...
فاز ۱ علاوه بر ۲۶ مدخل این‌ها را هم می‌آورد:

- سطر `architecture` در `data/topics.json`
- `data/architecture/categories.json` با هر ۷ دسته
- `data/architecture/roadmap.json` با **هر ۱۵ مرحله از همان اول**

`roadmap.json` کامل از روز اول ساخته می‌شود تا نقشهٔ کلی از ابتدا معلوم باشد. `pruneRoadmap` در `data.js` مرحله‌هایی را که مدخل‌هایشان هنوز وجود ندارند مدیریت می‌کند، پس مرحلهٔ خالی سایت را نمی‌شکند.

هفت فایل `entries/*.json` هم در فاز ۱ ساخته می‌شوند — بعضی‌شان با یک آرایهٔ کوتاه و بعضی خالی — تا `categories.json` به فایل ناموجود اشاره نکند.
