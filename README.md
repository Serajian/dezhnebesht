...
تست‌ها هم روی ماژول‌های خالص اجرا می‌شوند و هم روی دادهٔ واقعیِ روی دیسک. بعد `#/self-test` را باز کن: هر مدخل را در **هر دو زبان** رندر می‌کند و خطاهای رندر، خطاهای اعتبارسنجی و مدخل‌های بدون ترجمهٔ انگلیسی را گزارش می‌دهد.
...
این یک دانشنامهٔ شخصی است، ولی بسته نیست. دو جور می‌شود کمک کرد، و هر دو از [بخش Issues](https://github.com/Serajian/dezhnebesht/issues/new) شروع می‌شوند — اگر خودت JSON را می‌نویسی، مستقیم Pull Request بفرست.

### ۱. پیشنهاد یک مفهوم تازه

یک آبجکت به فایل دستهٔ مربوطه در `data/<موضوع>/entries/` اضافه می‌شود. ساختار کامل این است:

```json
{
  "id": "merkle-tree",
  "tags": ["structure", "integrity"],
  "related": ["hash", "blockchain"],
  "svg": [
    { "svg": "<svg viewBox=\"0 0 380 200\" direction=\"ltr\">…</svg>", "fa": "زیرنویس", "en": "caption" }
  ],
  "fa": {
    "title": "درخت مرکل",
    "short": "یک جمله — همان که در فهرست، زیر عنوان می‌نشیند",
    "body": "<p>…</p>",
    "example": "<p>…</p>"
  },
  "en": { "title": "Merkle Tree", "short": "…", "body": "<p>…</p>", "example": "<p>…</p>" }
}
```

فقط `fa.title` و `fa.short` و `fa.body` اجباری‌اند؛ بقیه اختیاری. `svg` هم می‌تواند یک رشته باشد، هم آرایه‌ای از چند دیاگرام با زیرنویس دوزبانه.

**موضوع و دسته فیلد نیستند** — موضوع از نام پوشه می‌آید و دسته از نام فایل. جای درست را از روی همان انتخاب کن. `id` باید در **کل سایت** یکتا باشد، نه فقط داخل موضوع.

اگر مطمئن نیستی متن را چطور بنویسی، فقط متن و مثال و شکل را در Issue بگذار — تبدیلش به این ساختار کار سختی نیست.

### قاعده‌هایی که مدخل باید رعایت کند

اینها سلیقه نیستند؛ هرکدام یک‌بار چیزی را شکسته‌اند:

- **اصطلاح تخصصی، بار اول، انگلیسی داخل پرانتز:** `اثر بهمنی <span dir="ltr">(Avalanche Effect)</span>`. یک‌بار در هر مدخل، نه هر بار.
- **فارسی هرگز داخل `<pre>` یا `<code>` نمی‌رود.** خط فارسی متصل است و فونت مونواسپیس اتصال حروف را پاره می‌کند. بلوک کد فقط لاتین و عدد.
- **SVGای که رشتهٔ لاتین یا هگز دارد، روی ریشه‌اش `direction="ltr"` می‌خواهد.** بدنهٔ مدخل RTL است و SVG ارثش می‌برد و آن رشته‌ها را جابه‌جا و قیچی می‌کند.
- **فرمول را در `<code>` ساده با فاصلهٔ معمولی بنویس.** جلوی شکستنِ وسط سطر را `render.js` خودش می‌گیرد؛ لازم نیست کاراکتر نامرئی بگذاری.
- **`related` را دوطرفه بنویس.** اگر مدخل تو به `hash` اشاره می‌کند، شناسهٔ تو هم باید به `related` همان اضافه شود.
- **هشتگ‌ها اسلاگ انگلیسیِ کوچک‌اند، بدون `#`** — تا یک تگ میان هر دو زبان مشترک بماند.
- **هر ادعای عددی باید حساب شده باشد.** تا امروز چند خطای عددی در متن‌های ورودی پیدا شده که همه‌شان باورپذیر بودند. اگر عددی می‌نویسی، بگو از کجا آمده.
...
بعد `#/self-test` را باز کن. هر مدخل را در هر دو زبان رندر می‌کند و خطای اعتبارسنجی، خطای رندر و مدخل بدون ترجمهٔ انگلیسی را نشان می‌دهد. شایع‌ترین خرابی، `related`ای است که به مدخلی هنوز نوشته‌نشده اشاره می‌کند.

### ۲. اصلاح یا تکمیل یک مدخل موجود

اگر جایی غلط دیدی، یا جایی هست که توضیحش کم است، **روی همان فیلد** درخواست بده. یعنی بگو کدام مدخل و کدام فیلد:

> در مدخل `hash`، در `fa.body`، نوشته شده «…» ولی درست این است که «…» — چون …

> در مدخل `private-key`، در `fa.example`، جای یک نمونهٔ عددی خالی است.

> در مدخل `generator-point`، دیاگرام دوم (`svg[1]`) این را نمی‌رساند که …

مسیر فیلد (`fa.body`، `en.example`، `svg[1]`) مهم است چون مدخل‌ها بلندند و بدون آن باید کل متن را دنبال اشاره‌ات گشت. اصلاحِ محتوایی — به‌خصوص عددی — با توضیح «چرا غلط است» خیلی سریع‌تر بررسی می‌شود تا با «این اشتباه است».
